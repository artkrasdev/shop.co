...
class CartItem extends HTMLElement {
    constructor() {
        ...
        const lineId = this.getAttribute('line_id');
        ...
        const apiCall = async (method, body) => {
            const url = `http://localhost:8888/SHOP.CO/backend/api/cart.php?line_id=${lineId}`;
        ...
        if (plus) plus.addEventListener('click', async () => {
            const newQty = parseInt(qtyVal.textContent) + 1;
            const cart = await apiCall('PUT', { qty: newQty });
            dispatch(cart);
        });
        ...
        if (minus) minus.addEventListener('click', async () => {
            ...
            if (current <= 1) return;
            const cart = await apiCall('PUT', { qty: current - 1 });
            dispatch(cart);
        });

        if (trash) trash.addEventListener('click', async () => {
            const cart = await apiCall('DELETE');
            dispatch(cart);
        });
    }
}
